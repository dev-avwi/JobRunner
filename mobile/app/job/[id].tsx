...
export default function JobDetailScreen() {
  ...
  const handleAdvancePhaseStatus = useCallback(async (phaseId: string, currentStatus: string) => {
    if (!id) return;
    setPhaseStatusLoading(prev => new Set(prev).add(phaseId));
    try {
      const res = await api.post<any>(`/api/jobs/${id}/phases/${phaseId}/advance-status`, {});
  ...
  const handleStatusChange = async () => {
    ...
    if (action.next === 'in_progress' && automationSettings?.requirePhotoBeforeStart) {
      ...
      if (beforePhotos.length === 0) {
        ...
        }).then((ok) => { if (ok) setActiveTab('files'); });
        return;
      }
    }
  ...
  const handleSaveDescription = async () => {
    ...
    const prev = job.description;
    setJob({ ...job, description: descriptionEditText.trim() });
    setEditingDescription(false);
    try {
      const response = await api.patch<Job>(`/api/jobs/${job.id}`, { description: descriptionEditText.trim() });
      if (response.data) setJob(response.data);
      showToast({ type: 'success', message: 'Description updated' });
    } catch {
      setJob({ ...job, description: prev });
      showToast({ type: 'error', message: 'Failed to update description' });
    } finally {
  ...
  // have been moved before the isLoading early return to satisfy Rules of Hooks.
  ...
  const renderOverviewTab = () => (
    <>
      ...
      {/* Main Action Button - Hero zone, prominent near the top */}
      ...
      {/* ── About this job ── */}
      {(!!job.description || isOwnerOrManager || isSoloOwner) && (
        <View style={{
          backgroundColor: colors.card,
          borderRadius: radius.xl,
          padding: spacing.lg,
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: spacing.sm, marginBottom: spacing.sm }}>
            ...
            <Text style={{ fontSize: typography.sizes.xs, fontWeight: fontWeights.bold, color: colors.mutedForeground, textTransform: 'uppercase', letterSpacing: 0.3, flex: 1 }}>About this job</Text>
            {!editingDescription && (isOwnerOrManager || isSoloOwner) && (
              <TouchableOpacity
                onPress={() => { setDescriptionEditText(job.description ?? ''); setEditingDescription(true); }}
          ...
          {editingDescription ? (
            <>
              <TextInput
                ...
                onChangeText={setDescriptionEditText}
                multiline
                placeholder="Add a job description..."
                placeholderTextColor={colors.mutedForeground}
                style={{ fontSize: typography.button.fontSize, color: colors.foreground, lineHeight: 22, minHeight: 80, textAlignVertical: 'top', borderWidth: 1, borderColor: colors.border, borderRadius: radius.md, padding: spacing.sm, marginBottom: spacing.sm }}
                autoFocus
              />
              <View style={{ flexDirection: 'row', gap: spacing.sm }}>
                <TouchableOpacity
                  onPress={() => setEditingDescription(false)}
                  style={{ flex: 1, alignItems: 'center', paddingVertical: 10, borderRadius: radius.lg, borderWidth: 1, borderColor: `${colors.primary}25`, backgroundColor: `${colors.primary}12` }}
                  activeOpacity={0.7}
                >
                  <Text style={{ fontSize: typography.button.fontSize, fontWeight: fontWeights.semibold, color: colors.primary }}>Cancel</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  onPress={handleSaveDescription}
                  disabled={isSavingDescription}
                  style={{ flex: 1, alignItems: 'center', paddingVertical: 10, borderRadius: radius.lg, backgroundColor: colors.primary, opacity: isSavingDescription ? 0.6 : 1 }}
                  activeOpacity={0.8}
                >
                  <Text style={{ fontSize: typography.button.fontSize, fontWeight: fontWeights.semibold, color: colors.primaryForeground }}>{isSavingDescription ? 'Saving...' : 'Save'}</Text>
                </TouchableOpacity>
              </View>
            </>
          ) : job.description ? (
            ...
            <TouchableOpacity onPress={() => { setDescriptionEditText(''); setEditingDescription(true); }} activeOpacity={0.7}>
              <Text style={{ fontSize: typography.button.fontSize, color: colors.mutedForeground, fontStyle: 'italic' }}>Tap to add a description...</Text>
            </TouchableOpacity>
          )}
        </View>
      )}
            ...
            <>
              <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: spacing.sm }}>
                {jobAssignments.map((assignment: any) => {
                  ...
                  const isApprovedLeave = hasLeave;
  ...
  const renderManageTab = () => (
    <>
      ...
      {/* Job Costing — compact summary, opens detailed costing sheet */}
      {(isOwnerOrManager || isSoloOwner) && (
        <TouchableOpacity
          activeOpacity={0.85}
          onPress={() => setShowJobCostingSheet(true)}
  ...
  return (
    <>
      ...
      {activeTab === 'tasks' && isProject && isTimerForThisJob && (
        <View style={{ marginHorizontal: pageShell.paddingHorizontal, marginBottom: spacing.sm, borderRadius: radius.lg, borderWidth: 1.5, borderColor: isOnBreak() ? colors.warning : colors.success, backgroundColor: isOnBreak() ? `${colors.warning}08` : `${colors.success}08`, paddingHorizontal: spacing.md, paddingVertical: spacing.sm }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: spacing.sm, flex: 1 }}>
              <Feather name={isOnBreak() ? 'coffee' : 'clock'} size={16} color={isOnBreak() ? colors.warning : colors.success} />
              <View style={{ flex: 1 }}>
                <Text style={{ fontSize: typography.caption.fontSize, color: isOnBreak() ? colors.warning : colors.success, fontWeight: fontWeights.semibold }} numberOfLines={1}>
                  {isOnBreak() ? 'Break' : 'Running'}{(() => { const ph = phases.find(p => p.id === (activeTimer as any)?.phaseId); return ph ? `: ${ph.phaseCode ? `${ph.phaseCode} — ` : ''}${ph.name}` : ''; })()}
                </Text>
                <Text style={{ fontSize: typography.body.fontSize, fontWeight: fontWeights.bold, color: isOnBreak() ? colors.warning : colors.success, fontVariant: ['tabular-nums'] }}>
                  {formatElapsedTime(elapsedTime)}
                </Text>
              </View>
            </View>
            <View style={{ flexDirection: 'row', gap: spacing.sm }}>
              <TouchableOpacity onPress={isOnBreak() ? handleResumeWork : handleTakeBreak} style={{ width: 34, height: 34, borderRadius: radius.md, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.muted, alignItems: 'center', justifyContent: 'center' }} activeOpacity={0.7}>
              ...
              <TouchableOpacity onPress={handleStopTimer} disabled={timerLoading} style={{ paddingHorizontal: spacing.md, height: 34, borderRadius: radius.md, backgroundColor: colors.destructive, alignItems: 'center', justifyContent: 'center', flexDirection: 'row', gap: spacing.xs, opacity: timerLoading ? 0.6 : 1 }} activeOpacity={0.7}>
                <Feather name="square" size={12} color={colors.primaryForeground} />
      ...
      >
        ...
        {activeTab === 'tasks' && (isProject ? (
          ...
          <>
            ...
            {(phases.length > 0 || isLoadingPhases) && (
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: spacing.sm, marginBottom: spacing.sm }}>
                <Text style={{ fontSize: 11, fontWeight: fontWeights.bold, color: colors.mutedForeground, textTransform: 'uppercase', letterSpacing: 0.6, flex: 1 }}>Work Plan</Text>
            ...
            {isLoadingPhases ? (
              ...
              if (isComplete && !isCompletedExpanded) {
                ...
                return (
                  <View
                    key={phase.id}
                    style={{ flexDirection: 'row', alignItems: 'center', paddingVertical: 10, paddingHorizontal: 2, marginBottom: 2, gap: spacing.sm }}
                  ...
                  >
                    ...
                    >
                      <View style={{ flexDirection: 'row', alignItems: 'center', gap: spacing.xs }}>
                        {phase.phaseCode ? (
                          <Text style={{ fontSize: 11, fontWeight: fontWeights.semibold, color: colors.mutedForeground }}>{phase.phaseCode}</Text>
                        ) : null}
                        <Text style={{ fontSize: typography.body.fontSize, color: colors.mutedForeground, flexShrink: 1, textDecorationLine: 'line-through' }} numberOfLines={1}>
                          {phase.name}
                        </Text>
                      </View>
                    </TouchableOpacity>
                ...
                <View key={phase.id} style={cardStyle} onLayout={(e) => { phaseLayoutYRef.current[phase.id] = e.nativeEvent.layout.y; }}>
                          ...
                          {phase.assignedUsers?.length ? (
                            <>
                              <View style={{ flexDirection: 'row' }}>
                                {phase.assignedUsers.slice(0, 3).map((member, idx) => (
                                  <View key={member.id} style={{ marginLeft: idx === 0 ? 0 : -6 }}>
                                    <TeamAvatar name={member.name} userId={member.id} size={18} />
                                  </View>
                                ))}
                              </View>
                              <Text style={{ fontSize: 11, color: colors.mutedForeground }}>
                                {phase.assignedUsers.slice(0, 2).map(m => m.name.split(' ')[0]).join(', ')}
                                {phase.assignedUsers.length > 2 ? ` +${phase.assignedUsers.length - 2}` : ''}
                              </Text>
                            </>
                          ) : phase.assignedUserName ? (
                            <>
                              <TeamAvatar name={phase.assignedUserName} userId={phase.assignedUserId || phase.assignedUserName} size={18} />
                              <Text style={{ fontSize: 11, color: colors.mutedForeground }}>{phase.assignedUserName.split(' ')[0]}</Text>
                            </>
                          ) : null}
                        </View>
                      ) : null}
                  ...
                  {!isComplete && (
                    <>
                      ...
                      {/* Phase status advance — workers tap to move forward */}
                      {(phase.status === 'not_started' || phase.status === 'in_progress') && (() => {
                        const isAdvancing = phaseStatusLoading.has(phase.id);
                        const nextLabel = phase.status === 'not_started' ? 'Mark In Progress' : 'Mark Complete';
                        const nextIcon: any = phase.status === 'not_started' ? 'arrow-right-circle' : 'check-circle';
                        const btnColor = phase.status === 'not_started' ? colors.primary : colors.success;
                        return (
                          <TouchableOpacity
                  ...
                  <TouchableOpacity
                    onPress={() => router.push({ pathname: '/job/phase-detail' as any, params: { jobId: String(id), phaseId: phase.id } })}
                    style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-end', gap: 4, paddingTop: spacing.sm, marginTop: spacing.xs, borderTopWidth: StyleSheet.hairlineWidth, borderTopColor: colors.border }}
                    activeOpacity={0.7}
                  >
                    <Text style={{ fontSize: 12, color: colors.primary, fontWeight: fontWeights.medium }}>View phase details</Text>
        ...
        {activeTab === 'manage' && (isOwnerOrManager || isSoloOwner) && (
          <>
      ...
      <AppBottomSheet
        ...
        <View>
          ...
          <TextInput
            style={[styles.singleLineInput, { height: 80, textAlignVertical: 'top' as any, paddingTop: spacing.sm, marginBottom: spacing.md }]}
