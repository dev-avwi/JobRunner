...
**Deployment PORT pin (TEMPORARY).** The production run command in `.replit [deployment]` is pinned to `sh -c "PORT=23636 NODE_ENV=production exec node dist/index.js"`. Reason: `.replit [[ports]]` has a stale orphan mapping `localPort 23636 -> externalPort 80` (left over from a removed canvas mockup-sandbox dev server) that demoted the real app to `5000 -> 5000`. The autoscale startup probe hits the container on the localPort bound to externalPort 80 (=23636) while the app would otherwise bind 5000 (`[env] PORT=5000`), so promote failed with a startup-probe timeout. The on-disk `[[ports]]` block cannot be edited by agent tools (direct edit blocked; `configureWorkflow` can't reclaim externalPort 80 while the orphan holds it; no tool deletes a workflow-less orphan port). **Footgun:** if the orphan `23636` mapping is ever cleaned (e.g. workspace restart) so externalPort 80 reverts to `localPort 5000`, this pin will break the next publish — at that point revert the run command to `["npm","run","start"]` (or `NODE_ENV=production node dist/index.js`). To fully fix: get the orphan port removed via Replit support / the Ports pane, then revert the run command so the app follows the canonical PORT again.
