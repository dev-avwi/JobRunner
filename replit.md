...
**WORKING fix (verified config, current as of 2026-05-30):**
```
...
```
`npm run start` runs `NODE_ENV=production node dist/index.js` (sets NODE_ENV inline). The app needs `NODE_ENV==='production'` (server/index.ts L617 `app.get("env")` gates `serveStatic` vs `setupVite`; L623 reads `PORT`). `NODE_ENV=production` is ALSO set in the production env store (redundant-but-harmless backstop).

**The build-command `rm -rf` cleanup IS REQUIRED again (see "image size over 8 GiB" runbook below) — but it is SCOPED.** The earlier breakage was caused SPECIFICALLY by deleting `.cache` and `.local`, which hold the files the deploy runtime layer needs to put the `nodejs-20` module on the PATH (proven: both `node` AND `npm` vanished together → whole module PATH destroyed). The current cleanup deletes ONLY build-ephemeral, non-runtime dirs and **never touches `.cache`/`.local`**, so the runtime PATH survives. **NEVER add `.cache`, `.local`, `node_modules`, `dist`, `package.json`, `.puppeteerrc.cjs`, or `.npmrc` to that `rm -rf` list.** The `rm` runs AFTER `npm run build`, so `attached_assets` (needed at build time via the vite `@assets` alias) is present during the build and only stripped from the final image. The cleanup runs on the deploy's ephemeral copy — it does NOT touch the user's real `mobile/` source or assets. (`bash -c` is fine for the BUILD command; the no-`bash -c` footgun below applies ONLY to the RUN command.)

**Ports / PORT (CORRECTED 2026-06-12):** the `[[ports]] 23636 -> 80` mapping and the `PORT=23636` prod env var were NOT the autoscale port — they were introduced by the `artifacts/mockup-sandbox` design artifact (created 2026-05-30, AFTER the last good deploy 75f29a8b on 2026-05-26, which had neither). An earlier runbook revision wrongly concluded they were the autoscale HTTP port and said to keep them. The mockup-sandbox artifact has been removed (2026-06-12): its directory, its `Component Preview Server` workflow, the prod `PORT` env var, and the lingering vite process on 23636 are all gone. The app binds `process.env.PORT`; autoscale injects `PORT` at runtime, so NO fixed `PORT` is needed in the prod env store. **Do NOT re-add `PORT=23636` to prod env or a `23636 -> 80` ports mapping.** (`.replit` `[[ports]]` entries are auto-managed by what's actually listening and can't be hand-edited; the stale 23636 entry clears once nothing binds it.)
...
**Deploy build fails "while bundling/building" but local build is clean — puppeteer Chrome download (FIXED 2026-05-30).** Symptom: autoscale publish fails during the build phase; `npm run build` succeeds locally (exit 0, ~1.5GB RAM, all artifacts present), deploy config is already the known-good one, and there are NO deployment *runtime* logs (it never reached runtime). Cause: `puppeteer@^24` has a `postinstall` (`node install.mjs`) that downloads its own Chrome (~150MB+) from Google during `npm install`; on the deploy builder that network fetch is the flaky step. The app never needs it — it runs against the Nix system `chromium` (in `[nix].packages`). Fix shipped: `.puppeteerrc.cjs` (`skipDownload: true` + `executablePath` resolved at runtime via `which chromium`, so every `puppeteer.launch()` defaults to the Nix browser) and `.npmrc` backstop `puppeteer_skip_download=true`. The 3 WHS PDF `puppeteer.launch()` calls in `server/routes.ts` were also pinned to system chromium inline (matching `server/pdfService.ts`). Verify with `node -e "console.log(require('puppeteer').executablePath())"` → prints the `/nix/store/...chromium` path. Do NOT remove `.puppeteerrc.cjs`/the `.npmrc` line — that re-introduces the fragile download into the deploy build.

**Deploy publish fails with `image size is over the limit of 8 GiB` (FIXED 2026-05-30).** Symptom: `npm run build` succeeds; the publish then fails at the "Created Repl layer" step with `error: image size is over the limit of 8 GiB`. This error appears in the **build/publish log the user sees**, NOT in `fetchDeploymentLogs()` (which returned "No deployment logs found" — it never reached runtime). Autoscale/Reserved-VM image cap is 8 GiB (static is 1 GiB). The image = the post-build repl filesystem snapshot. The known-good 2026-05-26 deploy was under the cap with no cleanup; the repl simply grew past 8 GiB since (biggest offenders measured 2026-05-30: `attached_assets` 1.3G, `node_modules` 1.1G, `.local` 885M, `mobile` 699M, `.git` 512M, `.cache` 395M). Fix: scoped `rm -rf` appended to the BUILD command (see "WORKING fix" above) deleting only build-ephemeral / non-runtime dirs — `attached_assets mobile .git docs tests exports artifacts`. Verified safe: `rg attached_assets server/` = no runtime reads (only build-time via vite `@assets` alias, so it MUST be deleted after `npm run build`, not before); no client/server imports from `mobile/` or `artifacts/`. `.replitignore` is NOT a documented deploy-image mechanism — deleting files (or external storage for large assets) is the documented way. If the image is STILL over 8 GiB after this, the next lever is moving `attached_assets` to object storage (rewrites the `@assets` imports — bigger change) — do NOT start deleting `.cache`/`.local`/`node_modules`.
