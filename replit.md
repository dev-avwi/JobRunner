...
**Cause:** the autoscale (Cloud Run) deploy container's exec context (the Go `pid1` runner AND any shell it spawns) has **neither `node` nor `npm` on PATH**. Those binaries exist in the image's nix store (`/nix/store/*-nodejs-*-wrapped/bin/`) but that dir is not on the run-time PATH — only standard dirs (`/usr/bin`, etc.) plus `bash`/`sh` are. So *every* run command that relies on PATH to find node/npm crash-loops with exit 127. Confirmed by three failed attempts, each in runtime logs via `fetchDeploymentLogs()`:
- `["sh","-c","... exec node ..."]` → `sh: exec: node: not found`
- `["npm","run","start"]` → `exec: "npm": executable file not found in $PATH` (Go runner can't even find npm)
- `["bash","-c","... node ..."]` (non-login bash doesn't source any profile) → `bash: line 1: node: command not found`

**Final fix (config only, no rebuild needed) — `deployConfig()` run command:**
```
["bash","-c","for d in /nix/store/*-nodejs-*-wrapped/bin; do PATH=\"$d:$PATH\"; done; export PATH; export NODE_ENV=production; exec node dist/index.js"]
```
`bash` IS present in the container; the loop globs the nodejs wrapped bin dir out of the nix store and prepends it to PATH (version-proof — survives a node bump, no hardcoded hash), then `exec node dist/index.js`. Verified locally: node lives at `/nix/store/1lagpgadaybvs1n2312gysg2phjk89y8-nodejs-20.20.0-wrapped/bin/node` (v20.20.0), which matches the glob.

**Required companion setting:** `PORT=23636` is set in the **production** env store (`setEnvVars({values:{PORT:"23636"},environment:"production"})`). Required because `.replit [[ports]]` has a stale orphan mapping `localPort 23636 -> externalPort 80` (leftover from a removed canvas mockup-sandbox dev server) — the autoscale HTTP port is whatever maps to externalPort 80 (=23636), confirmed by runtime log `forwarding local port 23636 to external port 80`. NOTE: `.replit [env] PORT="5000"` applies only to the **dev workspace**, NOT the deployment (prod env store had no PORT before this fix), so dev still uses 5000 and only prod uses 23636. `NODE_ENV=production` is set inline in the run command (not the env store).
...
**Why earlier theories were wrong:** (a) "startup-probe timeout from blocking seeding" — the code fix shipped but build `ab93c3dc` still failed identically; (b) the node/npm "not found" failures were all the same PATH problem, not a shell-choice problem — the only fix is to put the nix nodejs bin dir on PATH explicitly (or call node by absolute path).

**Footguns:**
- The orphan `[[ports]] 23636 -> 80` mapping cannot be edited by agent tools (direct `.replit` edit blocked; no port-mapping callback). If it's ever cleaned so externalPort 80 reverts to `localPort 5000`, the prod `PORT=23636` will break publishing — at that point delete the prod `PORT` var (app falls back to 5000). To fully clean up: remove the orphan `23636` port via the Ports pane (Task #190 tracks this), then delete the prod `PORT` env var.
- If the run command must reference node/npm directly again, remember PATH is bare — always glob the nix bin dir onto PATH first or use an absolute path.
