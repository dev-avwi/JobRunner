...
**Deployment promote failure — TWO independent causes (both must be fixed).** Publishing failed at the autoscale promote step (build succeeds, then `Creating Autoscale service` → ~1m45s → failed = startup-probe timeout). Root cause is a combination:

1. **Blocked startup port (PRIMARY, fixed in code).** `server.listen()` in `server/index.ts` only opened the HTTP port AFTER a long chain of awaited startup work — including demo-data seeding (`createDemoUserAndData` etc.) which runs in production whenever `ENABLE_DEMO_DATA="true"` (set in `.replit [userenv.production]`), plus a `configureTwilioWebhook` network call. On a cold/empty production DB this seeding is slow or can throw, so the port never opened before the health probe timed out. **Why local smoke tests passed:** dev has no `ENABLE_DEMO_DATA` and the dev DB is already seeded, so the block was skipped/instant. **Fix:** `configureTwilioWebhook` is now fire-and-forget, and demo-data seeding is deferred into the `server.listen()` callback (runs in background, wrapped in try/catch, non-fatal). The port now opens in ~10s regardless of DB state. Verified: `ENABLE_DEMO_DATA=true PORT=23636 NODE_ENV=production node dist/index.js` returns HTTP 200 in ~10s, with `[Demo] Demo data seeding complete` logged AFTER `serving on port`.

2. **Port pin (SECONDARY workaround).** Production run command in `.replit [deployment]` is pinned to `sh -c "PORT=23636 NODE_ENV=production exec node dist/index.js"`. Reason: `.replit [[ports]]` has a stale orphan mapping `localPort 23636 -> externalPort 80` (left over from a removed canvas mockup-sandbox dev server) that demoted the real app to `5000 -> 5000`. The autoscale probe hits the localPort bound to externalPort 80 (=23636), so the app must bind 23636 (it would otherwise bind 5000 via `[env] PORT=5000`). The on-disk `[[ports]]` block cannot be edited by agent tools (direct edit blocked; `configureWorkflow` can't reclaim externalPort 80 while the orphan holds it; no tool deletes a workflow-less orphan port). **Footgun:** if the orphan `23636` mapping is ever cleaned (e.g. workspace restart) so externalPort 80 reverts to `localPort 5000`, this pin will break the next publish — at that point revert the run command to `["npm","run","start"]`. To fully fix: get the orphan port removed via Replit support / the Ports pane, then revert the run command so the app follows the canonical PORT again.

Earlier debugging note: a publish (build `b4b4c509`, 2026-05-29 10:39) failed *with the pin already applied* — that was cause #1 (blocked port) masking the pin, which is how #1 was found.
