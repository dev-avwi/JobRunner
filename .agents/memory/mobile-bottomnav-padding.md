...
Addendum (Tap to Pay setup): a nested native-stack header shown on iOS INSIDE the padded layout eats ~56pt of content height, clipping bottom buttons under the tab bar; compensate with an IOS_HEADER_ALLOWANCE (20 was user-approved final; 56 floated too high) added to bottom padding of end containers. Also never toggle headerShown between steps of one route — iOS 26 animates the flip as a glass "morph" and mis-measures layout; keep it constant for the whole route.
