/**
 * timeEntryWrongPhase.test.ts
 *
 * API-level regression guard for POST /api/time-entries when the caller
 * supplies a phaseId that does not belong to the requested job.
 *
 * Verifies:
 *   1. The server returns 400 with "Phase not found or does not belong to
 *      this job" when phaseId exists but is from a different job.
 *   2. storage.getJobPhases IS called (the validation branch executes).
 *   3. storage.createTimeEntry is NOT called (the request is rejected before
 *      any write happens).
 */
...
const OWNER_ID  = "biz-owner-wp-1";
const WORKER_ID = "worker-wp-1";
const JOB_ID    = "job-wp-correct";

/** A phaseId that exists, but on a *different* job — not JOB_ID. */
const WRONG_PHASE_ID = "phase-from-other-job";
...
/** The job the worker is trying to clock into. */
const MOCK_JOB = {
  id:     JOB_ID,
  userId: OWNER_ID,
  status: "scheduled",
...
/**
 * getJobPhases returns phases for the correct job — none of which match
 * WRONG_PHASE_ID, simulating a phase that belongs to a different job.
 */
const PHASES_FOR_CORRECT_JOB = [
  { id: "phase-correct-1", jobId: JOB_ID, name: "Rough-in" },
  { id: "phase-correct-2", jobId: JOB_ID, name: "Fitoff" },
];
...
beforeEach(() => {
  ...
  // Worker is authenticated under OWNER_ID's business.
  ...
  // No active timer — avoids a 409.
  mockGetActiveTimeEntry.mockResolvedValue(null);

  // Job belongs to this business.
  mockGetJob.mockResolvedValue(MOCK_JOB);

  // Phases returned belong to JOB_ID — WRONG_PHASE_ID is not among them.
  mockGetJobPhases.mockResolvedValue(PHASES_FOR_CORRECT_JOB);

  mockGetJobAssignments.mockResolvedValue([]);
  mockGetMember.mockResolvedValue(null);
  mockGetBusinessSettings.mockResolvedValue(null);

  // createTimeEntry should never be reached in these tests, but set a safe default.
  mockCreateTimeEntry.mockResolvedValue(null);
});

// ── Tests ─────────────────────────────────────────────────────────────────────

describe("POST /api/time-entries — phaseId belongs to a different job", () => {
  ...
  it("calls storage.getJobPhases to validate the phaseId", async () => {
    ...
    expect(mockGetJobPhases).toHaveBeenCalledOnce();
  });

  it("does NOT call storage.createTimeEntry when the phase is rejected", async () => {
    const server = await buildApp();
