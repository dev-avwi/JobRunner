/**
 * timeEntryWithPhase.test.ts
 *
 * API-level regression guard for POST /api/time-entries when the caller
 * provides a valid phaseId (timer started after selecting a phase).
 *
 * Verifies:
 *   1. A timer started against a project job with a valid phaseId is accepted
 *      (201) — the server must store the entry with phase attribution.
 *   2. storage.getJobPhases is called to validate the phaseId.
 *   3. storage.createTimeEntry is called with the phaseId present in the
 *      payload, confirming the DB row would be stored with phase attribution.
 */
...
const OWNER_ID  = "biz-owner-2";
const WORKER_ID = "worker-2";
const JOB_ID    = "job-phase-happy";
const PHASE_ID  = "phase-abc-123";
...
/** A saved time entry returned by createTimeEntry — includes the phaseId. */
const SAVED_ENTRY_WITH_PHASE = {
  id:           "te-with-phase-1",
  userId:       WORKER_ID,
  jobId:        JOB_ID,
  phaseId:      PHASE_ID,
  ...
  endTime:      null,
  duration:     null,
  description:  "test timer with phase",
  timeCategory: "work",
  hourlyRate:   "85.00",
...
/** Phases list returned by getJobPhases — contains the phase being selected. */
const MOCK_PHASES = [
  { id: PHASE_ID, jobId: JOB_ID, name: "Phase 1" },
  { id: "phase-other-456", jobId: JOB_ID, name: "Phase 2" },
];
...
beforeEach(() => {
  ...
  // Phases list includes the requested phaseId
  mockGetJobPhases.mockResolvedValue(MOCK_PHASES);
  ...
  // Successful save returns the entry with the phaseId
  mockCreateTimeEntry.mockResolvedValue(SAVED_ENTRY_WITH_PHASE);
});

// ── Tests ─────────────────────────────────────────────────────────────────────

describe("POST /api/time-entries — timer started with a valid phaseId", () => {
  it("accepts the request and returns 201 when a valid phaseId is provided", async () => {
  ...
  it("calls storage.getJobPhases to validate the phaseId", async () => {
    const server = await buildApp();
    ...
    // The phase-validation branch must be entered when a phaseId is supplied
    expect(mockGetJobPhases).toHaveBeenCalled();
  });

  it("calls storage.createTimeEntry with the phaseId in the payload", async () => {
    const server = await buildApp();
    ...
    // phaseId must be present and equal to the one sent in the request
    expect(savedPayload.phaseId).toBe(PHASE_ID);
  });
});
