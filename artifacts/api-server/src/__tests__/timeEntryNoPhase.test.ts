/**
 * timeEntryNoPhase.test.ts
 *
 * API-level regression guard for POST /api/time-entries when the caller
 * provides no phaseId (timer started without selecting a phase).
 *
 * Verifies:
 *   1. A timer started against a project job with phaseId absent is accepted
 *      (200 / 201) — the server must NOT reject the request.
 *   2. storage.createTimeEntry is called without a phaseId in the payload
 *      (no phaseId key, or phaseId is null / undefined), confirming the
 *      DB row would be stored with no phase attribution.
 *   3. storage.getJobPhases is NOT called when phaseId is absent — the phase
 *      validation branch is correctly skipped.
 *   4. A timer started with an explicit phaseId=null is also accepted and
 *      stored without phase attribution (same behaviour as omitting the key).
 */
...
const OWNER_ID  = "biz-owner-1";
const WORKER_ID = "worker-1";
const JOB_ID    = "job-phase-test";
...
/** A saved time entry returned by createTimeEntry — no phaseId field. */
const SAVED_ENTRY_NO_PHASE = {
  id:           "te-no-phase-1",
  userId:       WORKER_ID,
  jobId:        JOB_ID,
  startTime:    new Date(PAST_START),
  endTime:      null,
  duration:     null,
  ...
  // phaseId is intentionally absent from this object
};
...
beforeEach(() => {
  ...
  // Default happy-path context: worker authenticated under OWNER_ID's business
  ...
  // Job is owned by this business
  mockGetJob.mockResolvedValue(MOCK_JOB);

  // No rate overrides — falls back to default 85.00
  mockGetJobAssignments.mockResolvedValue([]);
  mockGetMember.mockResolvedValue(null);
  mockGetBusinessSettings.mockResolvedValue(null);

  // Successful save returns the entry without a phaseId
  mockCreateTimeEntry.mockResolvedValue(SAVED_ENTRY_NO_PHASE);
});

// ── Tests ─────────────────────────────────────────────────────────────────────

describe("POST /api/time-entries — timer started with no phaseId", () => {
  it("accepts the request and returns 201 when phaseId is absent from the body", async () => {
    ...
    const res = await request(server)
      ...
      .send({
        ...
        // phaseId is intentionally omitted
      });

    expect(res.status).toBe(201);
  });

  it("calls storage.createTimeEntry without a phaseId when none is sent", async () => {
    const server = await buildApp();
    ...
    expect(mockCreateTimeEntry).toHaveBeenCalledOnce();

    const savedPayload = mockCreateTimeEntry.mock.calls[0][0];
    // phaseId must be absent or null — never a truthy value
    expect(savedPayload.phaseId == null).toBe(true);
  });

  it("does NOT call storage.getJobPhases when phaseId is absent", async () => {
    const server = await buildApp();
    ...
    // The phase-validation branch must be skipped entirely
    expect(mockGetJobPhases).not.toHaveBeenCalled();
  });

  it("accepts the request and omits phaseId when phaseId is explicitly null", async () => {
    const server = await buildApp();
    ...
    // getJobPhases must still be skipped — null is not a valid phaseId
    expect(mockGetJobPhases).not.toHaveBeenCalled();
  });
});
